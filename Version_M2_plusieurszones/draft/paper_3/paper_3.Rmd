...
Results_conv_2 <- summaryBy(Convergence+one~
  ...
  dplyr::select(n_samp_com,
                ...
                Convergence.sum,
                one.sum) %>%
    mutate(aggreg_obs = ifelse(aggreg_obs == T,"Dk","Yi"),
...
Results <- Results %>%
  mutate(relloc_aggreg = paste0("Reallocation: ",ifelse(reallocation==0,"No","Yes")," -  Likelihood: ",ifelse(aggreg_obs==T,"Dk","Yi"))) %>%
...
Results$relloc_aggreg <- factor(Results$relloc_aggreg,
                                levels = c("Reallocation: No -  Likelihood: Yi",
                                           ...
                                           "Reallocation: Yes -  Likelihood: Dk"))
...
Results_conv_2 <- Results_conv_2[-which(Results_conv_2$Model == "Scientific model" &
                                          Results_conv_2$`Likelihood level` == "Dk"),]
...
Results_plot$obs <- NA
Results_plot$obs[which(Results_plot$aggreg_obs == T)] <- "Dk"
...
Results_plot$obs <- factor(Results_plot$obs,levels = c("Yr","Dk","Scientific data"))
...
realloc_rest_plot <- ggplot(realloc_rest_df)+
  geom_point(aes(x=x,y=y,col=S_x),shape=15,size=2)+
  scale_color_distiller(palette = "Spectral",limits=c(0,NA))+
  ggtitle("Integrated model",subtitle = "Commercial likelihood on Dk")+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))+
  geom_sf(data = mapBase)+
...
## Parameter estimates
est_par_df_full_3$lkl[which(est_par_df_full_3$lkl == "Dj")] <- "Dk"

est_par_df_full_3$lkl[which(est_par_df_full_3$lkl == "Yi")] <- "Yr"
est_par_df_full_3$lkl <- factor(est_par_df_full_3$lkl,levels = c("Dk","Yr","Scientific model"))
