...
## An explicitly spatialized catch model
...
\begin{align}
P( Y_i = 0 | S(x_i) )& = \exp(-e^\xi S(x_{i}), \cr \nonumber
Y_i  | Y_i>0, S(x_i) & \sim \mathcal{L}{ogNormal}\left ( \frac{S(x_i)}{1-p_i} ,\sigma^2\right), \cr \nonumber
\end{align}
...
$$D_j | \mathcal{P}_j,\bm{S} \sim \mathcal{L}_D( \mu_j,\xi,\sigma^2)$$ 

with $\mathcal{P}_j=(x_1,...,x_i, ...,x_{m_j})$, the list of the fishing points associated to declaration $D_j$,  $\mu_j$ the expected positive biomass, $\xi$ the zero-inflation parameter and $\sigma^2$ the variance parameter. 
