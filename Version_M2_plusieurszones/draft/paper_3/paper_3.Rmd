...
In  @alglave_combining_2022, the authors propose a hierarchical spatial model to combine scientific survey data obtained through a standardized sampling protocol and catch data harvested by fishermen. This section provides a biref overview of the key ingredient of this model and raises some concern on the spatialization of the commercial catch data.

Let  $D\subset \R^2$ be a spatial domain and $\bm{S}=(S(x), x\in D)$ a spatial random field which represents the biomass for a species of interest.  $\bm{S}$ is assumed  to be a spatial log-Gaussian Random Field (GRF) defined as $\log(S(x))=\mu + \beta \, \Gamma(x) + \delta(x)$ (Figure \@ref(fig:SchemeRealloc)) where $\bm{\delta}=(\delta(x), x\in D)$ is a zero mean GRF with covariance matrix $\Sigma$.  As the catch are   zero-inflated positive continuous data,  following  @thorson_three_2018,  the authors model a catch at a given site $X_i$ with a mixture of a Dirac mass at $0$ and a Log Normal distribution, the proportion of the mixture being defined  conditionally on the random field $\bm{S}$.

\begin{equation}
\label{eq:defY}
Y_i | S(x_i)  \overset{ind}{\sim} \M{p_i}{\mu_i}{\sigma^2},
\end{equation}
with $p_i \defeq \exp(-e^\xi S(x_{i})$ the proportion of the mixture, $\mu_i\defeq 
\frac{S(x_i)}{1-p_i}$ the expected catch when positive and $\sigma^2$ its variance (this parametrization of a lognormal distribution is detailed in SMXXXX and corresponds to  


where $p_i\defeq \exp(-e^\xi S(x_{i}))$ is the mixture weight, $\mu_i \defeq \frac{S(x_i)}{1-p_i}$ is the mean of the lognormal distribution and $\sigma^2$ its variance, so that  
\begin{align}
...
In this approach all fishing locations $x_i$ and the coressponding catch are supposed to be known.
...
Actually, fishermen do not have to declare this precise information, but it is mandatory, for every vessel to declare the total daily catch aggregated at a given administrative spatial unit named  statistical rectangle. These data are recorded in logbooks data. For a given vessel, on a given day, a declaration (denoted $D_j$) is therefore the sum of all individual catch $Y_i$ realized in the unit associated with the catch  declaration $D_j$, more formally:

$$D_j=\sum_{x_i \in \mathcal{A}_j}{Y(x_i)},$$

where $\mathcal{A}_j$ is the administrative spatial unit associated to declaration $j$.    

The original model proposed in  @alglave_combining_2022 is defined at the individual catch scale, those catch are  derived from the catch declaration combined with the VMS data. This is classically done in fisheries science REFREFEREFREF by using a uniform reallocation process. This process consists in counting, for a given vessel, on a given day,  the number $m_j$ of fishing points in $\mathcal{A}_j$ and define for each $x_i\in\mathcal{A}_j,$ the associated individual catch $\Yr(x_i)\defeq D_j/m_j$. As a direct consequence, the so reconstructed individual catch are similar to a Catch Per Unit of Effort (CPUE), effort being measure in hours. As noted by @alglave_combining_2022, this process has several drawbacks.
First, as a consequence of the reallocation process, the reconstructed individual catches tend to exhibit smoother pattern than the original catches. Second, the actual number of data is the total number of catch declaration while the number of data after the reallocation process is the number of fishing locations, which is approximately 10 times the number of declarations. From a statistical point of view, this overestismation of the number of informative data tends to produce excessively narrow confidence intervals.
...
To circumvent such limitations, we propose an alternative approach that models the catch declarations $D_j$ instead of the reconstructed individual catch $\Yr(x_i)$. As we  aim to propose a model compatible with the original one, we are lead to specify  the distribution of $D_j$ which consists of a sum of $m_j$ random variables following a mixture distribution. This distribution has no  known analytical form. However the catch declaration data also exhibit some zero inflation and a long tail repartition of the values: a mixture model would be a good candidate. Therefore, we propose to use the same family of distribution for  $D_j$ than the one use for $\Yr$: a mixture between a Dirac mass in $0$ and a lognormal distribution. This is not exact, as the distribution of $D_j$ should be defined as convolution of mixtures which  would result in an untractable model. We somehow approximate the sum of  the original mixture distributions  by a mixture distribution of the same kind and we define the different key quantities, i.e  the mixture proportion, the expected positive catch declaration and its variance so that to match the original propostion:
...
$$D_j | \mathcal{P}_j,\bm{S} \sim \mathcal{L}_D( p^D_j, \mu^D_j, \sigma^{2,D})$$ 
...
(1) As the $Y_1, \ldots, Y_{m_j}$ are independent conditionally on $\bm{S}$,  the probability to obtain a zero-declaration $P(D_j = 0)$ is obtained by simply multiplying the probability to obtain a zero-punctual observation $P(Y_i=0 )$ to all fishing points $x_i \in \mathcal{P}_j$.

\begin{align*}
\P(D_j = 0\vert \bm{S}, x_1, \ldots x_{m_j})  & = \prod_{i\in \mathcal{P}_j} P(Y_{i} = 0\vert \bm{S}, x_i),\nonumber \\
...
$$\V ar(D_j \vert D_j > 0) = \frac{\sum_{i \in \mathcal{P}_j} \V ar(Y_{i})}{1-\pi_j} - \frac{\pi_j}{(1-\pi_j)^2}E(D_j)^2$$
$$\V ar(Y_{i})=\frac{S(x_{i})^2}{1-p_{i}}(e^{\sigma^2}-(1-p_{i}))$$



By using this second model, we expect to represent more accurately the observation process and avoid some of the drawbacks of the estimationbased on reallocated indivdual catch data $Y^r$.


The inference is realized through Template Model Builder (TMB), an effective tool to build hierarchical models and perform maximum likelihood estimation through automatic differentiation and Laplace Approximation [@kristensen_tmb_2016]. We made two simplifications. We use the Stochastic Partial Differential Equations (SPDE) approach to represent the spatiel field and we use integrated Nested Lappalace approximation to compute le likelihood. The stochastic random field is also approximated by a piecewise constant process defined on a fine grid.
...
## Comparison of the two approaches based on a simulation study

To evaluate the impact of reallocation on model outputs and how our alternative approach can improve model accuracy, we conduct a simulation study.
...
In these two sets of simulation studies, the covariate is modelled as a continous GRF that we suppose known at each point of the grid (when present the spatial random component is also a GRF but its values are not observed). The covariate effect is fixed to $\beta_S=2$ and the intercept is also fixed to $\mu=2$. Regarding commercial data, the number of fishing pings per declaration is fixed to 10 as it is the average number of fishing locations for a single declaration in real data. All parameterizations are detailed in the Table 1.
...
Two important variables may affect the accuracy of model outputs: the amount of commercial data and the number of fishing zones explored within one single declaration. In these simulations, we intend to explore the effect of these 2 variables.
