...
To circumvent such limitations, we propose an alternative approach that models the catch declarations $D_j$ instead of the individual catch $Y(x_i)$. As we  aim to propose a model compatible with the original one, we are lead to specify  the distribution of $D_j$ which consists of a sum of $m_j$ random variables folowwing a mixture distribution. This distribution has no  known analytical form. However the catch data also exhibit some zero inflation and a skewed repartition of the values and a mixture model would be a good candidate.  Theefore, we propose to model $D_j$ as a mixture between a Dirac mass in $0$ and a lognormal distribution. In order to compare the original approach with our proposition, we define the different key quantities, i.e  the mixture proportion, the expected positive catch declaration and its variance so that to match the original propostion:
