...
\begin{column}{0.3\textwidth}
Access to more data \\
Exact locations usually available
\end{column}
\begin{column}{0.3\textwidth}
...
\onslide<2>{
\footnotesize
\textbf{\textcolor{BaptisteBlue}{\ding{224}}} Some specific proposition in the context of fishery science: \\
\tiny
Alglave Baptiste, Rivot Etienne, Etienne Marie-Pierre, Woillez Mathieu, Thorson James T, Vermard Youen (2022). \textbf{Combining scientific survey and commercial catch data to map fish distribution.} ICES Journal of Marine Science IN PRESS. \url{https://doi.org/10.1093/icesjms/fsac032}
\vspace{\baselineskip}
}
...
\begin{columns}
\begin{column}{0.1\textwidth}
\center \tiny {\bf Resolution}
\end{column}
\begin{column}{0.45\textwidth}
\center \tiny Catch are daily declared at the resolution of ICES rectangles
\end{column}
\begin{column}{0.45\textwidth}
\center \tiny VMS pings are vessels GPS locations emitted each hour
\end{column}
\end{columns}
\tiny
\onslide<2>{
...
\begin{table}
  ...
  \textcolor{BaptisteRed}{\textbf{Actual situation}} & 
  $Y_i | \textcolor{BaptisteBlue}{S}(x_i), x_i \sim \mathcal{L}_Y(\textcolor{BaptisteBlue}{S}(x_i),\xi,\sigma)$          &
  $Y_i=\textcolor{BaptisteRed}{Y_i^*}$ \\
  \end{tabular}
\end{table}
...
\begin{table}
  ...
  $\textcolor{BaptisteLightGreen}{D_j} | \textcolor{BaptisteBlue}{S}_{\textcolor{BaptisteLightGreen}{\mathcal{P}_j}},\textcolor{BaptisteLightGreen}{\mathcal{P}_j} \sim \mathcal{L}_D( \textcolor{BaptisteBlue}{S}_{\textcolor{BaptisteLightGreen}{\mathcal{P}_j}},\xi,\sigma)$
...
\tiny
\begin{itemize}
\item \textcolor{BaptisteBlue}{\textbf{Latent field}} (\textcolor{BaptisteOrange}{covariate} + \textcolor{BaptisteBrown}{spatial random effect})
\item \textbf{Commercial data} (3000 samples over 2/3 of the area)
\item \textcolor{BaptisteLightGreen}{\textbf{Reallocation process}} (10 locations per declaration)
\item \textcolor{SurveyBlue}{\textbf{Scientific data}} (100 samples over the whole the area)
\end{itemize}
...
1/ Model fitted to \textcolor{SurveyBlue}{scientific data} (reference data) \\
\tiny \vspace{\baselineskip} \scriptsize
2/ Integrated model with \textcolor{BaptisteRed}{commercial likelihood built on $Y_i^*$} \\
\tiny \vspace{\baselineskip} \scriptsize
3/ Integrated model with \textcolor{BaptisteLightGreen}{commercial likelihood built on $D_j$} \\
\tiny \vspace{\baselineskip}
...
$$MSPE=\frac{\sum_x (\textcolor{BaptisteBlue}{S(x)} - \textcolor{BaptisteBlue}{\hat{S}(x)} ) ^2}{n}$$
...
\textbf{Commercial data:} OTB-DEF traulers (to ease convergence onboard observer data were integrated in fitting)\\
\textbf{Fitted models:} same as simulation\\
...
if(run_codes){
  ...
  ggsave("images/par_plot.png",width = 6,height = 7.5)
...
\begin{center}
\includegraphics[width=5cm]{images/par_plot.png}
\end{center}
\tiny *n.b.: punctual commercial data (i.e. observer data) needed to be integrated in inference in order to make the model converge and some parmaters needed to be step estimated (e.g. range)
...
\item Framework that integrates \textbf{catch declarations data} (rough resolution of catch) and \textbf{scientific/onboard observer data} (exact locations of catch) \\

\onslide<2-4>{

\textcolor{BaptisteBlue}{\ding{224}} \textbf{Still question:} how to ease convergence ? \\
...
\item \textbf{ Change of support issues } \\

\textbf{\textcolor{BaptisteBlue}{\ding{224}}} not so frequent in spatial ecology, \\

\textbf{\textcolor{BaptisteBlue}{\ding{224}}} but increasing amount of data available that may face such issues \\
...
\textbf{\textcolor{BaptisteBlue}{\ding{224}}} the overall approach is, \tiny \\ (i.e. modelling observed aggregagetd catches as a convolution of latent exact catches) \\ \vspace{\baselineskip} \normalsize

\textbf{\textcolor{BaptisteBlue}{\ding{224}}} but need to adapt the observation model to the data \tiny \\ (here zeroinflated continuous data)   \\ \vspace{\baselineskip} \normalsize

\textbf{\textcolor{BaptisteBlue}{\ding{224}}} and make the hypothesis that fishing locations ($\textcolor{BaptisteLightGreen}{\mathcal{P}_j}$) are known \scriptsize \\ (in reality $\textcolor{BaptisteLightGreen}{\mathcal{P}_j}$ is a trajectory and the set of fishing points are a realization of this trajectory) \\ \vspace{\baselineskip} \normalsize
