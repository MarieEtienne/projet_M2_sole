...
## Simulation plots
source("Scripts/source/simulation_plots.R")


## case study plots


file_vec <- c("fit_IM_res_int_Dj","fit_IM_res_int_Yi","fit_IM_res_int_Dj") # ,"realloc_int_df"
for(file_i in file_vec){
  
  print(file_i)
  load(paste0("draft/paper_3/res/",file_i,".RData"))
  ...
  if(str_detect(file_i,"no.realloc|Yi")) lkl <- "Yi"
  if(!str_detect(file_i,"no.realloc|Dj")) lkl <- "Dj"
